...
public class Sudoku {
	...
	public void checkLimitations() {
		// in the ideal case for every limitations all possibilities would be calculated and 
	...
	public void checkColumnAndRowCoverage() {
		...
		for (int il = 0; il < 9; il++) { // all columns as start
			for (int iu = il; iu < 9; iu++) { // rest columns as end
				...
				FieldSum tempC = new FieldSum((iu-il+1)*45,true); // Mark as unconnected - fields could have identical values
				FieldSum tempR = new FieldSum((iu-il+1)*45,true); // Mark as unconnected - fields could have identical values
				for (int j = 0; j < 9; j++) {
				...
				// search all limitations which are coverd completely by Fieldsum temp and substract them from temp
				for (FieldSum fstemp : givenlimitations) {
				...
				// Check if resulting Limitation leads to new results - store into calclimitations
				if (tempC.numFields() < 3 || tempC.sum < 45 ) {
					if (tempC.checkConnected()) calclimitations.add(tempC);
					System.out.println("Limitation with less than 3 Fields or sum < 45  "+tempC);
				}
				if (tempR.numFields() < 3 || tempR.sum < 45 ) {
					if (tempR.checkConnected()) calclimitations.add(tempR);

...
public class Tankaufgabe {
	
	public static final String eingabeFile = "fahrt1.txt";
	public static final String ausgabeFile = "ausgabeFahrt.txt";
	
	public static double v; // Verbrauch (in Liter) - auf 100 Km
	public static int g; // Tankgroesse (in Liter)
	public static double f; // verbleibende Tankfuellung (in Liter)
	public static int l; // Gesamtlaenge (in km)
		...
		if (l <= r) { // Es muss gar keine Tankstelle angefahren werden
			...
			while (aktiv == true) {
				...
				for (int i = 0; i < tankstellen.length; i++) {
					int mittelwert = (int) (((Double.parseDouble(tankstellen[i][0]) - Double.parseDouble(tankstellen[i][1]) * 25)) / 2) * 1000; // TODO: Bewertung anpassen
				...
				for (int i = tankstellen.length - 1; i >= 0; i--) { // Raenge (Rating) durchgehen
					for (int j = 0; j < tankstellen.length; j++) { // Fuer alle Tankstellen durchgehen
						 if (Integer.parseInt(tankstellen[j][3]) == i) { // Stimmt der Rang? (Rating)
							if (Integer.parseInt(tankstellen[j][0]) - gefahren <= r && Boolean.parseBoolean(tankstellen[j][4]) != true) { // Liegt die Tankstelle auch in Reichweite?
								...
								f = f - ((v / 100) * (r - Integer.parseInt(tankstellen[j][0]))); // Update: Tankf�llung, wenn wir ankommen
								...
								if (g  >= ((v / 100) * (l - gefahren))) { // Kommen wir mit weniger als Volltanken bis zum Ziel?
									tanken = (l - gefahren) * (v / 100); // Benoetigte Liter bis zum Ziel
									aktiv = false;
								} else {

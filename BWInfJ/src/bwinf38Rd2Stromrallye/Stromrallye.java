...
public class Stromrallye {
	...
	int maxSolutions = -1; // Maximale Anzahl von Lösungen
	final static int maxSolutionsFinalStatic = 100000;
	...
	public void solve() {
		if (maxSolutions == -1) maxSolutions = maxSolutionsFinalStatic;
		robl++;
		gesamtl++;
		backtrack(robx, roby);
		robl--;
	...
	private boolean backtrack(int xpos, int ypos) { // neue Position des Roboters
		// prüfen ob Position zulaessig
		if (xpos < 0 || xpos >= size || ypos < 0 || ypos >= size || maxSolutions == 0)
			return false;
		robl--;
		gesamtl--;
		if (gesamtl == 0) { // Erfolgreiche Lösung
			System.out.println(weg);
			if (maxSolutions>=0) maxSolutions--;
			robl++; // und zurück
			gesamtl++;
			return true;
		}
		boolean battery_taken = false;
		if (spielfeld[xpos][ypos] > 0) { // Batterie tauschen
			int t = robl;
			robl = spielfeld[xpos][ypos];
			spielfeld[xpos][ypos] = t;
			weg += "B";
			battery_taken = true;
		}
		if (robl > 0) { // weitergehen
			weg += "N";
			backtrack(xpos, ypos - 1); // nach Norden
			weg = weg.substring(0, weg.length() - 1);
			weg += "S";
			backtrack(xpos, ypos + 1); // nach Süden
			weg = weg.substring(0, weg.length() - 1);
			weg += "E";
			backtrack(xpos + 1, ypos); // nach Osten
			weg = weg.substring(0, weg.length() - 1);
			weg += "W";
			backtrack(xpos - 1, ypos); // nach Westen
			weg = weg.substring(0, weg.length() - 1);
		}
		// Rückweg antreten
		if (battery_taken) { // Batterie zurücktauschen - auch wenn leer!
			weg = weg.substring(0, weg.length() - 1);
			int t = robl;
			robl = spielfeld[xpos][ypos];
			spielfeld[xpos][ypos] = t;
		}
		// Ladung wieder aufladen
		robl++;
		gesamtl++;
		return false;
	}
